...
$(function(){
    ...
    if(flag){
        $('.tit').html(orgName)
        $(document).attr("title",orgName);
        console.log(GetQueryString('orgName'))
        getAjax().done(function (res) {
            init();
            $("#goActiveList").show().attr('href','activeList.html?merchantId='+merchantId+'&userId='+userId+'&orgName='+encodeURI(GetQueryString('orgName')))
        })
    }
    function getAjax(){
        if(!flag){
